...
std::pair<int, int> runOnce() {
    ...
    for (int i = 0; i < numPipes; ++i) {
        ...
        channel.setReadCallback([capture0 = channel.fd(), i](auto &&PH1) {
            return readCallback(std::forward<decltype(PH1)>(PH1), capture0, i);
        });
    ...
    auto beforeLoop = std::chrono::system_clock::now();
    g_loop->loop();

    // Timestamp end(Timestamp::now());
    auto end = std::chrono::system_clock::now();

    int iterTime = (std::chrono::duration_cast<std::chrono::milliseconds>(end - beforeInit)).count();
    int loopTime = (std::chrono::duration_cast<std::chrono::milliseconds>(end - beforeLoop)).count();
