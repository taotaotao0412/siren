...
class Client : noncopyable {
   public:
    ...
    void onConnect() {
        ...
        if (numConnected_.load() == sessionCount_) {
            LOG_WARN("all connected");
        }
    }

    void onDisconnect(const TcpConnectionPtr& conn) {
        LOG_WARN("disconnected {}", numConnected_.fetch_sub(1));
        if (numConnected_.load() == 0) {
            ...
            LOG_WARN("{} tatal bytes read", totalBytesRead);
            LOG_WARN("{} total messages read", totalMessagesRead);
            LOG_WARN("{} average message size",
    ...
    ~Client(){
        LOG_INFO("pingpong Client::~Client()");
    }
...
int main(int argc, char* argv[]) {
    if (argc != 7) {
        ...
        Client *client = new Client(&loop, serverAddr, blockSize, sessionCount, timeout,
                      threadCount);
        loop.loop();
